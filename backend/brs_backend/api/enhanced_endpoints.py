...
async def check_section_attachable(request: AttachabilityRequest, db: Session = Depends(get_db)):
...
async def generate_recommendations(request: RecommendationRequest, db: Session = Depends(get_db)):
    ...
    try:
        result = get_schedule_recommendations(request.student_id, request.target_credits)
        if not result["success"]:
            raise HTTPException(status_code=400, detail=result["error"])
        return result["data"]
    except Exception as e:
...
async def submit_recommendation_feedback(request: FeedbackRequest, db: Session = Depends(get_db)):
...
async def get_student_schedule(student_id: int, term_id: Optional[UUID] = None, db: Session = Depends(get_db)):
...
async def detect_schedule_conflicts(student_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {
            ...
            "recommendations": [
                "No conflicts detected in your current schedule."
            ]
        }
