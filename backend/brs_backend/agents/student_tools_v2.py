...
def check_schedule_attachable(section_id: str, student_id: int) -> dict:
    ...
    try:
        ...
        conflicts = db.execute(
            conflict_query, (section_id, student_id, student_id)
        ).fetchall()
        if conflicts:
            ...
            violations.append(
                {
                    ...
                    "details": {"prerequisites": section_info["prerequisites"]},
                }
            )
...
def get_schedule_recommendations(
    student_id: int, target_credits: Optional[int] = None
) -> dict:
...
def record_recommendation_feedback(
    rec_id: str, student_id: int, feedback: str, notes: str = None
) -> dict:
...
def _get_student_busy_times(db, student_id: int, term_id: str = None):
    ...
    return db.execute(
        query, (student_id, student_id, term_id, term_id, term_id)
    ).fetchall()
